...
def find_match_helper(template, target):
...
def find_match(img1, img2):
    ...
    first_temp, first_target = find_match_helper(img1, img2)
    second_temp, second_target = find_match_helper(img2, img1)

    same = {}
    pts1 = []
    pts2 = []
    for i in range(len(first_temp)):
        same[tuple(first_temp[i])] = first_target[i]

    for i in range(len(second_temp)):
        if (tuple(second_target[i]) in same and same[tuple(second_target[i])][0] == second_temp[i][0] and
                same[tuple(second_target[i])][1] == second_temp[i][1]):
            pts1.append(second_target[i])
            pts2.append(second_temp[i])
...
def compute_F(pts1, pts2):
    ...
    for i in range(800):
        ...
        for j in range(8):
            A[j, 0] = pts1[index[j]][0] * pts2[index[j]][0]
            A[j, 1] = pts1[index[j]][1] * pts2[index[j]][0]
            A[j, 2] = pts2[index[j]][0]
            A[j, 3] = pts1[index[j]][0] * pts2[index[j]][1]
            A[j, 4] = pts1[index[j]][1] * pts2[index[j]][1]
            A[j, 5] = pts2[index[j]][1]
            A[j, 6] = pts1[index[j]][0]
            A[j, 7] = pts1[index[j]][1]
            A[j, 8] = 1

        F = null_space(A)[:, 0].reshape(3, 3)
        U, D, V_t = np.linalg.svd(F)
        D[2] = 0
        F = np.matmul(U * D, V_t)

        for k in range(len(pts1)):
            single_loss = np.matmul(np.matmul(np.array([pts2[k][0], pts2[k][1], 1]), F),
                                 np.array([pts1[k][0], pts1[k][1], 1]))
            loss.append(single_loss * single_loss)
...
def triangulation(P1, P2, pts1, pts2):
    ...
    for i in range(len(pts1)):
        skew1 = np.array([[0, -1, pts1[i][1]], [1, 0, -pts1[i][0]], [-pts1[i][1], pts1[i][0], 0]])
        skew2 = np.array([[0, -1, pts2[i][1]], [1, 0, -pts2[i][0]], [-pts2[i][1], pts2[i][0], 0]])
        A = np.vstack((np.matmul(skew1, P1)[:2], np.matmul(skew2, P2)[:2]))
...
def disambiguate_pose(Rs, Cs, pts3Ds):
    ...
    for i in range(len(Rs)):
        valid = 0
        cur_c = Cs[i].reshape(-1)
        cur_r = Rs[i][2]
        for j in range(len(pts3Ds[i])):
            cam1 = (pts3Ds[i][j] - cur_c)
            cam2 = np.dot(pts3Ds[i][j] - cur_c, cur_r)
            if cam1[2] > 0 and cam2 > 0:
                valid += 1
        # list.append(valid)
        if valid > most_num:
            most_num = valid
...
def compute_rectification(K, R, C):
    ...
    rz_hat = np.array([0, 0, 1])
    C = C.reshape(-1)
    rx = C / np.linalg.norm(C)
    rz = (rz_hat - np.matmul(rz_hat, rx)*rx) /np.linalg.norm(rz_hat - np.matmul(rz_hat, rx)*rx)
    ry = np.cross(rz,rx)
    r_rect = np.array([rx,ry,rz])
    H1 = np.matmul(np.matmul(K, r_rect),np.linalg.inv(K))
    H2 = np.matmul(np.matmul(np.matmul(K, r_rect),R.T), np.linalg.inv(K))
...
def dense_match(img1, img2):
    ...
    for i in range(len(img1)):
        for j in range(len(img1[0])):
            keypoints.append(cv2.KeyPoint(x=j, y=i, _size=2))
    key1, des1 = sift.compute(img1, keypoints)
    key2, des2 = sift.compute(img2, keypoints)

    des1 = np.array(des1).reshape((len(img1),len(img1[0]),128))
    des2 = np.array(des2).reshape((len(img1), len(img1[0]), 128))
    ...
    for i in range(len(img1)):
        for j in range(len(img1[0])):
            if img1[i][j] == 1:
                ...
                dist = []
                for k in range(j+1):
                    dist.append(np.linalg.norm(target - des2[i][k]))
                disparity[i][j] = abs(np.argmin(dist)-j)
...
if __name__ == '__main__':
    ...
    img_left_w = cv2.resize(img_left_w,
                            (int(img_left_w.shape[1] / 2), int(img_left_w.shape[0] / 2)))  # resize image for speed
